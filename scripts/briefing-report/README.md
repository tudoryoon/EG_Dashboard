...
`Publish Weekly Briefing PDF` runs on Saturday at 11:00 KST (02:00 UTC).
...
- `workflow_dispatch` defaults to `dry_run: true`: it tests the current snapshot and uploads a temporary preview without publishing. Disable dry run only when the reporting week's data is complete. The regular Saturday schedule publishes automatically.
